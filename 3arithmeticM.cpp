...
void select2(node& n) 
{
    ...
    n.display.push_back("x" + string(8 - to_string(n.first).size(), ' ') + to_string(n.first) + "   x");
    n.display.push_back("x " + op + string(5 - to_string(n.second).size(), ' ') + to_string(n.second) + "   x");
...
void update2(node& n)
{
    ...
    n.display.push_back("x " + op + string(5 - to_string(n.second).size(), ' ') + to_string(n.second) + "   x");
    n.display.push_back("x --------- x");
    n.display.push_back("x" + string(8 - ans.size(), ' ') + ans + "   x");
    n.display.push_back("x           x"); 
    n.display.push_back("xxxxxxxxxxxxx");           
}

void deselect2(node& n)
{
    string ans = to_string(n.attempt);
    char o = n.display[2][2];
    string op = "  ";
    ...
    n.display.clear();
    n.display.push_back("-------------");
    n.display.push_back("|" + string(8 - to_string(n.first).size(), ' ') + to_string(n.first) + "   |");
    n.display.push_back("| " + op + string(5 - to_string(n.second).size(), ' ') + to_string(n.second) + "   |");
    n.display.push_back("| --------- |");
    n.display.push_back("|" + string(8 - ans.size(), ' ') + ans + "   |");
    n.display.push_back("|           |"); 
    n.display.push_back("-------------");           
}

void submit2(node test[4][6])
{   
    int right = 0;
    float grade;
    vector<node> wrong;
    ...
    if (wrong.empty()) {
        ...
        for (int i = 0; i < wrong.size(); i++) {
            node& n = wrong[i];
            for (int j = 0; j < 7; j++) {
                if (j == 4) cout << "|" + string(8 - to_string(n.answer).size(), ' ') + to_string(n.answer) + "   |" << endl;
...
void arithmetic2()
{
    ...
    // introduction
    cout << "Welcome to Arithmetic Practice (Medium)." << endl;
